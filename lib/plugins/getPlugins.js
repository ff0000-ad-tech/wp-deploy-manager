...
module.exports = function getPlugins({ DM, PM, buildEntry, fbaTypes }) {
	...
	if (!DM.deploy.get().output.debug) {
		...
		plugins.push(
			new UglifyJsPlugin({
				// For some reason, the listed options under Webpack's included Uglify plugin don't work since
				// console.logs weren't removed
				// However, these options from the following reference have removed console.logs
				// Reference: https://stackoverflow.com/questions/41040266/remove-console-logs-with-webpack-uglify
				compress: {
					drop_console: true,
					warnings: false
				},
