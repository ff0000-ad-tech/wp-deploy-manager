...
const EventCallbackWebpackPlugin = require("event-callback-webpack-plugin")
  .default;
  ...
  const plugins = [
      ...
      [
        {
          context: path.normalize(
            `${DM.ad.get().env.build}/${DM.ad.get().paths.common.context}`
          ),
          from: "**/*",
    ...
    new CopyWebpackPlugin(
      [
        {
          context: path.normalize(
            `${DM.ad.get().env.build}/${DM.ad.get().paths.ad.context}`
          ),
          from: "**/*",
          to: ""
        }
    ...
    new AssetsPlugin(DM, {
      ...
      fbaTypes,
      output: {
        path: `${DM.deploy.get().output.context}/${
          DM.deploy.get().output.folder
        }`,
        filename: base64Inline ? "" : "fba-payload.png",
        hasFbaAssets: filename => {
          DM.ad.get().settings.ref.assets.payload.binary = filename
            ? [filename]
            : [];
        }
      },
        ...
        },
        {
          ...
          payload: function() {
            return {
              modules: DM.ad
                .get()
                .settings.ref.assets.failover.images.map(obj => {
                  return obj.source;
                })
            };
          },
          copy: {
            ...
            to: `${DM.deploy.get().output.context}/${
              DM.deploy.get().output.folder
            }`
          }
        },
    ...
    new IndexPlugin(DM, {
      source: {
        path: `${DM.ad.get().env.build}/${DM.ad.get().paths.ad.context}/${
      ...
      inject: {
        "ad-global": `${
          DM.ad.get().env.build
        }/node_modules/@ff0000-ad-tech/ad-global/dist/ad-global.inline.js`,
        promise_polyfill:
          "./node_modules/promise-polyfill/dist/polyfill.min.js",
        commons: `${
          DM.ad.get().env.build
        }/node_modules/@ff0000-ad-tech/ad-load/dist/${
          debug ? "umd.debug.js" : "umd.min.js"
        }`
      },
      output: {
        path: `${DM.deploy.get().output.context}/${
