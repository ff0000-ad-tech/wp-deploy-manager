...
module.exports = function getPlugins({ scope, DM, PM, base64Inline }) {
	...
	const plugins = [
		...
		new AssetsPlugin(DM, {
			aggregators: [
				{
					...
					store: (asset) => {
						DM.payload.watchAsset('build', 'images', asset)
					},
					setter: (assets) => {
				...
				},
				{
					...
					store: (asset) => {
						DM.payload.watchAsset('build', 'fonts', asset)
					},
					setter: (assets) => {
			...
			// base64Inline, // NOTE: this was previously an option for FBA-compiling
			emitters: [
				// images
				{
					copy: {
				...
				// for reference, preloader gets inlined, not emitted
				// {
				// 	inline: {
				// 		from: DM.payload.store.get('inline')
				// 	}
				// }
			]
		}),
