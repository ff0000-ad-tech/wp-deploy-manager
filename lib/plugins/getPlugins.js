...
const COPY_IGNORE_FOLDERS = ['js/**/*', 'components/**/*', 'styles/**/*', 'fonts/**/*', 'images/**/*']
	...
	const plugins = [
		...
		new AssetsPlugin(DM, {
			fba: {
				...
				base64Inline,
				output: {
					context: path.resolve(outputContext, DM.deploy.get().output.folder),
					filename: 'fba-payload.png'
				},
				setPayloadReq: (filename) => {
					DM.deploy.get().settings.assets.payload.binary = filename ? [filename] : []
				}
			},
			assets: [
				{
